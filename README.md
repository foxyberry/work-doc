# 📄 **Project Documentation**


## 🚀 Continuous Integration and Continuous Deployment
- [⚙️ Automating Docker Builds](./ci/cd/automating-docker-builds.md)


## 🐳 Docker Configuration
- [🔨 Multi-Stage Dockerfile](./docker/multi-stage-dockerfile/README.md)
- [📖 Multi-Stage Dockerfile (Korean)](./docker/multi-stage-dockerfile/KO_KR.md)


## 🔐 Keycloak Authentication Integration
- [🔑 Keycloak](./keycloak/README.md)
- [🇰🇷 Keycloak (Korean)](./keycloak/KO_KR.md)


## ⚙️ Miscellaneous Configuration
- [🐍 Python Version in JupyterLab](./etc/add-python-version-to-jupyterlab.md)


## 🧠 Language Processing Chains
### 💬 Chatbots
- [🚀 Quick Start](./lang-chain/chatbots/quick-start.md)
- [🔍 Retrieval](./lang-chain/chatbots/retrieval.md)
- [🛠️ Tool Usage](./lang-chain/chatbots/tool-usage.md)


### 🗂️ Memory Management
- [📝 Memory Management Documentation](./lang-chain/memory-management/README.md)


### 🤖 Tool Use and Agents
- [📘 Agent Tools Quick Start](./lang-chain/tool-use-and-agents/quick-start.md)
